...
import { SENDER_NAME_LIMIT, SUBJECT_LIMIT, validateNotificationContent } from "@/lib/notification-validation";
import {
...
export default function HomeScreen() {
  ...
  const [scheduledSeconds, setScheduledSeconds] = useState<number | null>(null);
  const [feedback, setFeedback] = useState<{ kind: "success" | "error"; message: string } | null>(null);

  const refresh = useCallback(async () => {
    const [nextPermission, nextPreferences, draft, nextTemplates] = await Promise.all([
      getNotificationPermission(),
      readPreferences(),
      consumeDraft(),
      readTemplates(),
  ...
  const pickNotificationImage = async () => {
    ...
    if (!result.canceled && result.assets[0]?.uri) {
      ...
      await saveDraft(senderName, subject, { senderName, imageUri: result.assets[0].uri });
    }
  };

  const handleSaveTemplate = async () => {
    const validation = validateNotificationContent(senderName, subject);
    if (!validation.valid) {
    ...
    setTemplates((current) => [template, ...current].slice(0, 20));
    setTemplateName("");
  ...
  const applyTemplate = (template: NotificationTemplate) => {
    ...
    void saveDraft(template.senderName, template.subject, { senderName: template.senderName, imageUri: template.imageUri });
  ...
  const handleEmit = async () => {
    ...
    try {
      ...
      const notificationId = await emitLocalNotification(
        ...
        validation.subject,
        { senderName: validation.senderName, imageUri },
        scheduledSeconds || undefined
      );
      await addToHistory({
        id: `${Date.now()}`,
        subject: validation.subject,
      ...
      await saveDraft(senderName, subject, { senderName, imageUri });
      if (preferences.hapticsEnabled) haptic.success();
      setFeedback({
        kind: "success",
        message: scheduledSeconds 
          ? `Notificação agendada para daqui a ${scheduledSeconds / 60} min.` 
          : "Notificação emitida com sucesso."
      });
      setScheduledSeconds(null);
    } catch (error) {
  ...
  const toneColor = copy.tone === "success" ? colors.success : copy.tone === "warning" ? colors.warning : colors.primary;
    ...
    <ScreenContainer edges={["top", "left", "right"]} containerClassName="bg-background">
      ...
      >
        ...
        >
          ...
          <View style={[styles.formCard, { backgroundColor: colors.surface, borderColor: colors.border }]}>
            <View style={styles.inputGroup}>
              ...
              <TextInput
                ...
                onEndEditing={() => void saveDraft(senderName, subject, { senderName, imageUri })}
                placeholder="Ex.: Minha equipe"
                placeholderTextColor={colors.muted}
                maxLength={SENDER_NAME_LIMIT}
                returnKeyType="next"
                style={[styles.textInput, styles.senderNameInput, { color: colors.foreground, borderColor: colors.border }]}
              />
            </View>
            <View style={[styles.separator, { backgroundColor: colors.border }]} />
            <View style={styles.inputGroup}>
              <View style={styles.labelRow}>
                <Text style={[styles.inputLabel, { color: colors.foreground }]}>Assunto da notificação</Text>
                <Text style={[styles.counter, { color: colors.muted }]}>{subject.length}/{SUBJECT_LIMIT}</Text>
              </View>
              <TextInput
                value={subject}
                onChangeText={setSubject}
                onEndEditing={() => void saveDraft(senderName, subject, { senderName, imageUri })}
                placeholder="Ex.: Entre na sala em até 5 minutos."
                placeholderTextColor={colors.muted}
                maxLength={SUBJECT_LIMIT}
                multiline
                textAlignVertical="top"
                returnKeyType="done"
                style={[styles.textInput, styles.multilineInput, { color: colors.foreground, borderColor: colors.border }]}
              />
            </View>
          </View>

          <View style={[styles.imageCard, { backgroundColor: colors.surface, borderColor: colors.border }]}>
            <Pressable onPress={() => void pickNotificationImage()} style={({ pressed }) => [styles.imagePicker, pressed && styles.pressed]}>
              {imageUri ? (
              ...
              <View style={styles.imagePickerCopy}>
                <Text style={[styles.imagePickerTitle, { color: colors.foreground }]}>{imageUri ? "Imagem personalizada" : "Imagem da notificação"}</Text>
                <Text style={[styles.imagePickerText, { color: colors.muted }]}>{imageUri ? "Toque para trocar a imagem." : "Escolha uma imagem para o preview."}</Text>
              </View>
              <MaterialIcons name="chevron-right" size={21} color={colors.muted} />
            </Pressable>
            {imageUri ? (
              <Pressable onPress={() => { setImageUri(null); void saveDraft(senderName, subject, { senderName, imageUri: null }); }} style={({ pressed }) => [styles.removeImageButton, pressed && styles.pressed]}>
          ...
          <View style={styles.sectionHeader}>
            <Text style={[styles.sectionTitle, { color: colors.foreground }]}>Agendamento</Text>
            <Text style={[styles.sectionHint, { color: colors.muted }]}>ESCOLHA O MOMENTO</Text>
          </View>

          <View style={[styles.formCard, { backgroundColor: colors.surface, borderColor: colors.border, flexDirection: 'row', flexWrap: 'wrap', gap: 8, padding: 12 }]}>
            {[
              { label: 'Agora', value: null },
              ...
              <Pressable
                key={option.label}
                onPress={() => {
                  setScheduledSeconds(option.value);
                  if (preferences.hapticsEnabled) haptic.light();
                }}
                style={({ pressed }) => [
                  {
                    ...
                    borderRadius: 10,
                    borderWidth: 1,
                    borderColor: scheduledSeconds === option.value ? colors.primary : colors.border,
                    backgroundColor: scheduledSeconds === option.value ? `${colors.primary}15` : 'transparent',
                  },
                  pressed && styles.pressed
                ]}
              >
                <Text style={{ 
                  fontSize: 13, 
                  fontWeight: '700', 
                  color: scheduledSeconds === option.value ? colors.primary : colors.muted 
                }}>
                  {option.label}
                </Text>
              </Pressable>
          ...
          <View style={[styles.templatesCard, { backgroundColor: colors.surface, borderColor: colors.border }]}> 
            <View style={styles.templateNameRow}>
              ...
              <Pressable
                ...
                disabled={!isReady}
                style={({ pressed }) => [styles.saveTemplateButton, { backgroundColor: isReady ? colors.primary : `${colors.primary}55` }, pressed && isReady && styles.buttonPressed]}
              >
            ...
            <FlatList
              ...
              ListEmptyComponent={<Text style={[styles.emptyTemplateText, { color: colors.muted }]}>Seus modelos salvos aparecerão aqui.</Text>}
              renderItem={({ item }) => (
                <Pressable
                  onPress={() => applyTemplate(item)}
                  style={({ pressed }) => [styles.templateChip, { backgroundColor: `${colors.primary}12`, borderColor: `${colors.primary}30` }, pressed && styles.pressed]}
                >
                  {item.imageUri ? <Image source={{ uri: item.imageUri }} style={styles.templateChipImage} /> : <MaterialIcons name="notifications-none" size={17} color={colors.primary} />}
                  <View style={styles.templateChipCopy}>
                    <Text numberOfLines={1} style={[styles.templateChipName, { color: colors.foreground }]}>{item.name}</Text>
                    <Text numberOfLines={1} style={[styles.templateChipSubject, { color: colors.muted }]}>{item.subject}</Text>
                  </View>
          ...
          <View style={styles.notificationPreviewStage}>
            <View style={styles.notificationGlassCard}>
              <View style={[styles.notificationAppIcon, { backgroundColor: imageUri ? "transparent" : colors.primary }]}>
                {imageUri ? <Image source={{ uri: imageUri }} style={styles.notificationImage} /> : <MaterialIcons name="notifications-none" size={21} color="#FFFFFF" />}
              </View>
              <View style={styles.notificationCopy}>
                <View style={styles.notificationMetaRow}>
                  <Text numberOfLines={1} style={styles.notificationTitle}>{previewSenderName}</Text>
                  <Text style={styles.notificationTime}>agora</Text>
                </View>
                <Text numberOfLines={2} style={styles.notificationSubject}>{previewSubject}</Text>
              </View>
            </View>
          ...
          {feedback ? (
            ...
            >
              ...
              />
              <Text style={[styles.feedbackText, { color: feedback.kind === "success" ? colors.success : colors.error }]}>
                {feedback.message}
              </Text>
            </View>
          ) : null}
          ...
          >
            {isSending ? <ActivityIndicator color="#FFFFFF" /> : <MaterialIcons name="notifications-active" size={21} color="#FFFFFF" />}
          ...
          <Text style={[styles.footerNote, { color: colors.muted }]}>A emissão acontece localmente, sem precisar de servidor.</Text>
        </ScrollView>
      </KeyboardAvoidingView>
    </ScreenContainer>
  );
...
const styles = StyleSheet.create({
  ...
  brandMark: { width: 44, height: 44, borderRadius: 15, overflow: "hidden", shadowColor: "#102A43", shadowOpacity: 0.16, shadowRadius: 8, shadowOffset: { width: 0, height: 4 }, elevation: 4 },
  ...
  heroCard: { backgroundColor: "#102A43", borderRadius: 26, padding: 22, gap: 10, overflow: "hidden", shadowColor: "#102A43", shadowOpacity: 0.18, shadowRadius: 16, shadowOffset: { width: 0, height: 8 }, elevation: 6 },
  heroTopline: { flexDirection: "row", alignItems: "center", gap: 10 },
  heroIconCircle: { width: 42, height: 42, borderRadius: 15, alignItems: "center", justifyContent: "center", backgroundColor: "#0F766E" },
  ...
  permissionCard: { minHeight: 72, borderRadius: 19, borderWidth: 1, padding: 13, flexDirection: "row", alignItems: "center", gap: 11 },
  ...
  templateNameInput: { flex: 1, minHeight: 43, borderRadius: 12, borderWidth: 1, paddingHorizontal: 11, paddingVertical: 9, fontSize: 13, lineHeight: 18 },
  saveTemplateButton: { minHeight: 43, borderRadius: 12, paddingHorizontal: 11, flexDirection: "row", alignItems: "center", justifyContent: "center", gap: 5 },
  ...
  templateChip: { minWidth: 205, maxWidth: 245, borderRadius: 15, borderWidth: 1, padding: 10, flexDirection: "row", alignItems: "center", gap: 8 },
  ...
  removeImageButton: { borderTopWidth: 1, borderTopColor: "#D7E1E8", marginTop: 9, paddingTop: 10, flexDirection: "row", alignItems: "center", gap: 5 },
  ...
  notificationGlassCard: { borderRadius: 22, borderWidth: 1, borderColor: "rgba(255,255,255,0.24)", padding: 14, flexDirection: "row", gap: 12, backgroundColor: "rgba(27, 39, 52, 0.78)", shadowColor: "#000000", shadowOpacity: 0.22, shadowRadius: 18, shadowOffset: { width: 0, height: 8 }, elevation: 8 },
  ...
  notificationPreviewNote: { color: "rgba(255,255,255,0.66)", fontSize: 10, lineHeight: 14, marginTop: 9, textAlign: "center" },
  ...
  primaryButton: { minHeight: 56, borderRadius: 18, paddingHorizontal: 17, flexDirection: "row", alignItems: "center", justifyContent: "center", gap: 10, shadowColor: "#0F766E", shadowOpacity: 0.22, shadowRadius: 13, shadowOffset: { width: 0, height: 7 }, elevation: 5 },
