...
export default function HistoryScreen() {
  ...
  const reuse = async (item: NotificationRecord) => {
    haptic.light();
    await saveDraft(item.senderName, item.subject, { senderName: item.senderName, imageUri: item.imageUri });
    router.replace("/(tabs)");
  };
          ...
          renderItem={({ item }) => (
            <View style={[styles.historyCard, { backgroundColor: colors.surface, borderColor: colors.border }]}>
              ...
              <View style={styles.itemCopy}>
                ...
                <Text numberOfLines={1} style={[styles.itemTitle, { color: colors.foreground }]}>{item.senderName || "Nome exibido"}</Text>
                <Text numberOfLines={2} style={[styles.itemSubject, { color: colors.muted }]}>{item.subject}</Text>
              </View>
